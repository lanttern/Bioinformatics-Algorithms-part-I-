...
def all_pattern(genome, k):    #function to generate all pattern with k-mer
    ...
    return pattern
'''
def Motifs(array, k, t):    #function to generate all pattern with k-mer
    allpattern = []
    motifs = []
    for i in range (0, t):
        allpattern.append(all_pattern(array[i], k))
    for element in allpattern:
        Nt_pr = matrix(element)
        motif = Profile_most(Nt_pr, element)
        motifs.append(motif)  
    print motif
    return motifs
'''    
...
def GreedySearch(array, k, t):
    ...
    return bestmotifs, score(bestmotifs)#, score(['CAG', 'CAG', 'CAA', 'CAA', 'CAA']), score(['CAG', 'CAG', 'CAA', 'CAC'])
  
array = []
with open("7_4test1.txt", "r") as r:
    for line in r:
