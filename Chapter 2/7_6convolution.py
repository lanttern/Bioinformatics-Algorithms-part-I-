...
def Spectral_Convolution (Spectrum):
    ...
    for i in range(0, len(Spectrum)):
        for j in range (0, len(Spectrum)-1):
            if Spectrum[i] > Spectrum[j] and Spectrum[i]-Spectrum[j] in mass_AA:
                list1.append(Spectrum[i]-Spectrum[j])
                count1.append(list1.count(Spectrum[i]-Spectrum[j]))
    count2 = list(count1)
    count2 = set(count2)
    count2 = list(count2)
    count2.sort()
    #print count2
    for n in range(0, len(count1)):
        if count1[n] >=count2[250]:
            final_list.append(list1[n])
    #final_list.sort()
    return final_list
